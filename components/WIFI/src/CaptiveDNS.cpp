...
static void  captdnsRecv(int sockFd,struct sockaddr_in *premote_addr, char *pusrdata, unsigned short length) {

	char* buff = (char*)malloc(DNS_LEN);
	if(buff == nullptr) {
		printf("Cannot allocate buffer for captive DNS\n");
		return;
	}
	char* reply = (char *)malloc(DNS_LEN);
	if(reply == nullptr) {
		printf("Cannot allocate reply buffer for captive DNS\n");
		free(buff);
		return;
	}
	int i;
   ...
   // 		my_ntohs(&hdr->id), hdr->flags, hdr->rcode, my_ntohs(&hdr->qdcount), my_ntohs(&hdr->ancount), my_ntohs(&hdr->nscount), my_ntohs(&hdr->arcount), length);
	...
	if (length>DNS_LEN ||								//Packet is longer than DNS implementation allows
	    ...
	    hdr->ancount || hdr->nscount || hdr->arcount ||	//this is a reply, don't know what to do with it
        hdr->flags&FLAG_TC) { //truncated, can't use this
			printf("Failed\n");
			free(buff);
			free(reply);
	...
	for (i=0; i<my_ntohs(&hdr->qdcount); i++) 
	{
		...
		printf("DNS: Q (type 0x%X class 0x%X) for %s\n", my_ntohs(&qf->type), my_ntohs(&qf->nsclass), buff);
...
void CaptiveDns::ProcessRequest() {
	...
	switch(select (FD_SETSIZE,&set, NULL, NULL, &timeout)) {
		case -1:
			printf("Error selected from captive DNS socket\n");
			break;
		case 0 : 
			return;
		case 1 :
			...
			ret=recvfrom(_sockFd, (uint8_t *)msg, DNS_LEN, 0,(struct sockaddr *)&from,(socklen_t *)&fromlen);
			if (ret>0) captdnsRecv(_sockFd, &from,msg,ret);
			break;
	}
